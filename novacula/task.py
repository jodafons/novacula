...
class Script:
    ...
    def submit(self, dry_run : bool=False) -> int:
        ...
        if not dry_run:
            result = subprocess.run(['sbatch', script_path], stdout=subprocess.PIPE, text=True)
            if result.returncode != 0:
                raise RuntimeError(f"Failed to submit script: {result.stderr}")
            # Extract the job ID from the output
            job_id = int(result.stdout.split()[-1])  # Assuming the job ID is the last word in the output
            return job_id
        else:
            return -1
...
class Task:
    ...
    def run(self , dry_run : bool=False, virtualenv : str=""):
        ...
        script += f"#SBATCH --array=0-{nfiles}"
...
class Session:
    ...
    def run(self, dry_run : bool=False):
        ...
        for task in __tasks__.values():
            ...
            task.run( dry_run=dry_run, virtualenv = self.virtualenv )
